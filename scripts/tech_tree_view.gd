...
const POSITIONS = {
	"precision": Vector2(10, 45), "module_slots": Vector2(10, 235), "mixed_mode": Vector2(10, 425), "advanced_instruments": Vector2(10, 615),
...
func _ready() -> void:
	custom_minimum_size = Vector2(1020, 795)
	for key in simulation.UPGRADES:
		...
		body.add_child(LabUI.label(spec.description, 11, LabUI.MUTED, true))
		var action = LabUI.button("", func(): simulation.unlock_upgrade(key); refresh(), "upgrade")
		action.add_theme_stylebox_override("normal", LabUI.box(LabUI.PANEL, LabUI.LINE, 7))
...
func refresh() -> void:
	for key in controls:
		...
		var blocked = spec.requires != "" and spec.requires not in simulation.unlocked
