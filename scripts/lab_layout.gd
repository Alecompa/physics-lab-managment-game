...
const REST_SEATS = [Vector2i(13, 11), Vector2i(14, 11), Vector2i(16, 11), Vector2i(17, 11)]
...
static func fixed_furniture(cell: Vector2i) -> bool:
	return cell.y == 10 and cell.x in [13, 14, 16, 17] or cell.y == 9 and cell.x in [13, 14, 15, 16, 17]

static func footprint(kind: String) -> Vector2i:
	return Vector2i(2, 1) if kind == "desk" else Vector2i(2, 2)
...
static func room_allows(kind: String, origin: Vector2i) -> bool:
	for cell in cells(kind, origin):
		...
		if kind != "desk" and room(cell) not in ["optics", "measurements"]: return false
